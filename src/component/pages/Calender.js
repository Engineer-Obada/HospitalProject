...
const Calender = () => {
  ...
  const loadFields = async()=>{
      const result = await axios.get("http://192.168.242.6:8000/api/operatingRoom/show")
      console.log(result.data.data);

      const result1 = await axios.get("http://localhost:3002/FormReserv")

      
      setRooms(result.data.data)
      setOpertion(result1.data)
...
const showOperationsRooms = (d)=>{
  ...
  dataRoom = opertion.filter(({date,content})=> date === dataCell && content === d )
...
const getListData = (value) => {
  ...
  data = Rooms.filter(({Date})=> Date === dateValue )
  return data || [];
  ...
  const dateCellRender = (value) => {
    ...
    <ul className="events">
      {listData.map((item) => (
        <>
        <li className='ss'  key={item.Type} onClick={ e=>showOperationsRooms(item.content)}>
          ...
          <i className='expand bi bi-person-check-fill btn btn-primary  btn-sm '  onClick={ showModalDoctor}></i>
  ...
  const handelSelected = (e)=>{
    ...
    const stringValue = e.format("yyyy-MM-DD");
    dataCell = stringValue
    data = opertion.filter(({date})=> date === stringValue )
  ...
  <>
      ...
      <div>
        <Menu mode="horizontal" defaultSelectedKeys={['2']}>
          ...
          <Menu.Item key={"3"} icon={<FieldTimeOutlined />} onClick={ showModalTime}>
          AddRoomOpertion
          </Menu.Item>
      ...
      <Modal width={10000} title="Basic" visible={isModalVisibleRoom} onOk={handleOkRoom} onCancel={handleCancelRoom}>
      <Table   columns={columnsOper} dataSource={dataRoom}/>
      </Modal>
